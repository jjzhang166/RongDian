...
OnQuit是用于资源释放，由于目前子面板的管理方式较为特殊，子面板实例使用全局定义，所以子面板的析构函数可能会比主窗体的析构慢(也就是主窗口已经执行玩自己的析构函数了，但是由于子面板实例为全局定义，此时子面板的析构函数可能还没有调用，而子面板的资源却又从属与主窗体的)，因此，为了释放子面板的资源，这里增加一个接口，由主窗体资源释放时主动调用，主动清理子面板所申请的资源
