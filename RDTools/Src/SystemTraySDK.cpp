...
TCHAR CSystemTray::m_szInvisibleName[MAX_PATH] = {0};
...
ATOM CSystemTray::RegisterClass(HINSTANCE hInstance)
{
	...
	wcex.lpszMenuName	= 0;
	wcex.lpszClassName	= kRDToolsTray_Class;
	wcex.hIconSm		= 0;
                       ...
                       UINT uBalloonTimeout /*=10*/)
{
	...
	// For FindWindow Can't find the HWND after calling MinimiseToTray
	memset(m_szInvisibleName, 0, sizeof(m_szInvisibleName));
	_tcscpy(m_szInvisibleName, kRDToolsStaticClass);
    ...
    m_hWnd = ::CreateWindow(kRDToolsTray_Class, _T(""), WS_POPUP, 
...
BOOL CSystemTray::RemoveTaskbarIcon(HWND hWnd)
{
    ...
    {
		m_hWndInvisible = CreateWindowEx(0, _T("Static"), m_szInvisibleName, WS_POPUP,
