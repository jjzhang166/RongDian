...
BOOL CConfigTableDB::Query(LPCONFIG_TABLE lpTable)
{
	...
	CSQLiteQuery query = m_pSQLite->ExecQuery("select count(*) from config;");
	m_nRows = atoi(query.FieldValue(0));
	query = m_pSQLite->ExecQuery(szSQL_u8);
	if(m_pResults)
		delete[] m_pResults;
	m_pResults = NULL;
	m_pResults = new CONFIG_TABLE[query.NumFields()];
	if(!m_pResults)
		return FALSE;
	int i = 0;
	while(!query.Eof())
	{
		StrUtil::u82u(query.FieldValue("name_"), m_pResults[i].szName);
		StrUtil::u82u(query.FieldValue("value_"), m_pResults[i].szValue);
		i++;
		query.NextRow();
	}
