...
std::string PrintMD5(uchar md5Digest[16], bool uppercase)
{
	std::ostringstream md5Str;
	md5Str.setf(std::ios_base::hex,std::ios_base::basefield);
	if(uppercase)
		md5Str.setf(std::ios_base::uppercase);
	else
		md5Str.unsetf(std::ios_base::uppercase);
...
// MD5Str: Performs the MD5 algorithm on a std::string, returning
// the results as a std::string.
std::string MD5Str(std::string& str, bool uppercase /*= false*/)
{
	...
	return PrintMD5(alg.Digest(), uppercase);
}

// MD5File: Performs the MD5 algorithm on a file (binary or text),
// returning the results as a char*.  Returns NULL if it fails.

/* Commented Out For Anepsosis.... fopen_s seems to have issues with gcc */
std::string MD5File(std::string& filename, bool uppercase /*= false*/)
{
	FILE* file;
	md5 alg;
	int nLen;
...
std::wstring MD5WStr(std::wstring& /*str*/, bool /*uppercase*/ /*= false*/)
{
	return L"";
}

std::wstring MD5WFile(std::wstring& /*filename*/, bool /*uppercase*/ /*= false*/)
{
	return L"";
}
