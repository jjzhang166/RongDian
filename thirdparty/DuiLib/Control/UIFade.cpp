...
namespace DuiLib {
	...
	CFadeUI::CFadeUI(CControlUI* pOwner)
	{
		ASSERT(pOwner != NULL);
		m_pControl = pOwner;
	}
	...
	BOOL CFadeUI::StartAnimation(int nElapse, int nTotalFrame, int nAnimationID /*= 0*/, BOOL bLoop/* = FALSE*/)
	{
	...
	VOID CFadeUI::StopAnimation(int nAnimationID /*= 0*/)
	{
	...
	BOOL CFadeUI::IsAnimationRunning(int nAnimationID)
	{
		CAnimationData* pData = GetAnimationDataByID(nAnimationID);
		return NULL != pData;
	}

	int CFadeUI::GetCurrentFrame(int nAnimationID/* = 0*/)
	{
		CAnimationData* pData = GetAnimationDataByID(nAnimationID);
		if( NULL == pData )
		{
	...
	BOOL CFadeUI::SetCurrentFrame(int nFrame, int nAnimationID/* = 0*/)
	{
		CAnimationData* pData = GetAnimationDataByID(nAnimationID);
		if( NULL == pData)
		{
	...
	VOID CFadeUI::OnAnimationStart(int nAnimationID, BOOL bFirstLoop)
	{

	}

	VOID CFadeUI::OnAnimationStep(int nTotalFrame, int nCurFrame, int nAnimationID)
	{

	}

	VOID CFadeUI::OnAnimationStop(int nAnimationID)
	{

	}

	VOID CFadeUI::OnAnimationElapse(int nAnimationID)
	{
		if(m_pControl == NULL) return;
	...
	CAnimationData* CFadeUI::GetAnimationDataByID(int nAnimationID)
	{
