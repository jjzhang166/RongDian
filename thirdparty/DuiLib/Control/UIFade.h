...
namespace DuiLib {
	...
	class UILIB_API IFadeUI
	{
	public:
		virtual ~IFadeUI() { NULL; }
	...
	template class __declspec(dllexport) std::allocator<CAnimationData*>;
	template class __declspec(dllexport) std::vector<CAnimationData*, std::allocator<CAnimationData*> >;

	class UILIB_API CFadeUI: public IFadeUI
	{
	public:
		CFadeUI();
		CFadeUI(CControlUI* pOwner);
		~CFadeUI();
