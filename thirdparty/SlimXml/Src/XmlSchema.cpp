...
///////////////////////////////////////////////////////////////////////////////////////////////////\r\n");
	...
	int pos1 = (int)definition.find_last_of(T('/'));
	int pos2 = (int)definition.find_last_of(T('\\'));
	int slashPos = std::max(pos1 == (int)String::npos ? -1: pos1,
								pos2 == (int)String::npos ? -1: pos2);
	...
	file.write((char*)bom, (std::streamsize)3);
	file.write(buffer, (std::streamsize)characterCount);
	delete[] buffer;
#else
...
bool XmlSchema::parseNodeStruct(XmlNode* dst, XmlNode* src)
{
		  ...
		  child = src->getNextChild(nodeIterator))
	{
		...
		if (structure == NULL)
		{
			...
			while (parent != NULL)
			{
				if (Stricmp(parent->getName(), child->getName()) == 0)
				{
					recursive = true;
					break;
				}
...
const Char* XmlSchema::guessType(const Char* content) const
{
	...
	if (Stricmp(content, T("true")) == 0
		|| Stricmp(content, T("false")) == 0)
	{
		return T("bool");
	}
								 ...
								 String& readingFunction, String& writingFunction) const
{
	...
	if (defaultAttribute != NULL)
	{
		...
		if (Stricmp(type->getString(), T("string")) == 0)
		{
...
void XmlSchema::addConstructorItem(const XmlNode* child, String& structDefinition, int& index) const
{
	...
	XmlAttribute* defaultAttribute = child->findAttribute(ATTR_DEFAULT);
	if (type != NULL
		&& Stricmp(type->getString(), T("string")) == 0
...
{
	if (Stricmp(type, T("string")) == 0)
	{
		return LEFT_QUOTE T("\"");
	}
	if (Stricmp(type, T("bool")) == 0)
	{
		return T("false");
	}
	if (Stricmp(type, T("int")) == 0)
	{
		return T("0");
	}
	if (Stricmp(type, T("float")) == 0)
	{
		return T("0.0f");
	}
	return T("deadbeef");
