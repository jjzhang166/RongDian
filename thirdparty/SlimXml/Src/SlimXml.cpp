...
{
	return (Stricmp(m_value, T("true")) == 0);
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
XmlNode* XmlNode::findChild(int index) const
{
	assert(index >= 0);
	int i = 0;
...
XmlNode* XmlNode::findChild(const Char* name) const
{
		  ...
		  ++iter)
	{
		XmlNode* child = *iter;
		assert(child != NULL);
		if (Stricmp(child->m_name, name) == 0)
		{
			return child;
		}
	}
...
XmlNode* XmlNode::addChild(XmlNode* before_node, const Char* child_name, NodeType type)
{
	if (type != COMMENT && type != ELEMENT)
	{
		return NULL;
	...
	XmlNode* iter_child = NULL;
	size_t count = 0;
	for (NodeList::iterator iter = m_children.begin(); iter != m_children.end(); ++iter)
	{
		iter_child = *iter;
		assert(iter_child != NULL);
		++count;
		if (iter_child == before_node)
		{
			new_child = new XmlNode(type, this);
			if (child_name != NULL)
				new_child->setName(child_name);
			m_children.insert(iter, new_child);
		}
	}
	return new_child;
}
...
XmlAttribute* XmlNode::findAttribute(const Char* name) const
{
		  ...
		  ++iter)
	{
		...
		if (Stricmp(attribute->getName(), name) == 0)
		{
			return attribute;
		}
	}
...
void XmlNode::readAttributeAsIntArray(const Char* name, int* out, unsigned long length, int defaultValue) const
{
	...
	unsigned long stringLen = (unsigned long)Strlen(attribute->getString());
	...
	while (index < length)
	{
		...
		stringLen -= (unsigned long)(found - current + 1);
		current = found + 1;
		++index;
		if (found == NULL)
		{
										   ...
										   unsigned long enumCount, unsigned long defaultValue) const
{
	...
	{
		if (Stricmp(enumNames[i], attribute->getString()) == 0)
		{
			return i;
		}
	}
...
bool XmlDocument::loadFromStream(std::istream& input)
{
	...
	input.read(buffer, (std::streamsize)size);
...
bool XmlDocument::save(const Char* filename, Encode encode) const
{
	...
	if (encode == UTF_16)
	{
		...
		file.write((char*)output.c_str(), (std::streamsize)(output.size() * 2));
	}
	else if (encode == UTF_16_BIG_ENDIAN)
	{
		bom[0] = 0xfe;
		...
		file.write((char*)buffer, (std::streamsize)(output.size() * 2));
		delete[] buffer;
	}
	else if (encode == UTF_8 || encode == UTF_8_NO_MARK)
	{
		...
		file.write(tempBuffer, (std::streamsize)converted);
		delete[] tempBuffer;
	}
#else
	#if defined (UNICODE)
...
size_t utf16toutf8(const wchar_t* u16, size_t size, char* u8, size_t outBufferSize)
{
	...
	while (size > 0)
	{
		...
		else
		{
			...
			*u8 = (char)((*u16 >> 12) | 0xe0);
			*(u8+1) = (char)(((*u16 >> 6) & 0x3f) | 0x80);
			*(u8+2) = (char)((*u16 & 0x3f) | 0x80);
			u8 += 3;
			++u16;
			--size;
			converted += 3;
