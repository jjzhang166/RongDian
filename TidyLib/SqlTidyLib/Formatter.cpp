 ...
 * ParameterList:	int n_maxLookahead, int n_indent, bool n_endLineOnSubQuery, int n_minSubqueryLength, char n_indentationChar
 * Parameter:       n_maxLookahead:最大先行
 * Parameter:       n_indent:缩进空格数
 * Parameter:       n_endLineOnSubQuery:在子查询后换行
 * Parameter:       n_minSubqueryLength:最小子查询长度
 * Parameter:       n_indentationChar:以什么字符缩进
 * Return Value:	
 * Date:        	13:09:10 00:05:36
 * Author:			LIng
 * CopyRight:		
 ************************************************************************/
SQLFormatter::SQLFormatter( int n_maxLookahead /*= 2*/, int n_indent /*= 2*/, bool n_endLineOnSubQuery /*= true*/, int n_minSubqueryLength /*= 100*/, char n_indentationChar /*= ' '*/ )
{
...
bool SQLFormatter::_isSqlToken(const char *start, const char *end)
{
...
void SQLFormatter::_output(const char *start, const char *end, bool endLine)
{
	start;
	end;
	endLine;
...
void SQLFormatter::_formatSql(char *start, char *end, char *dest, int indent, bool lastWasToken)
{
	char *p1 = start;
	char *p2 = start;
	char *res = dest;
	...
	//cerr << "flushing buffer zone (length:" << end - p1 << ") : ";
	//_output(p1, end, true);
...
void SQLFormatter::_addIndentation(char *&p, int indent)
{
	_addIndentation(p, indent, _indentationChar);
}

// Adds indentation characters to given input
void SQLFormatter::_addIndentation(char *&p, int indent, char indentationChar)
{
...
void SQLFormatter::_moveWhileSpaces(char *&p)
{
	for (; *p == ' ' || *p == '\n' || *p == '\r' || *p == '\t'; *p++) {}
}
...
void SQLFormatter::_moveUntil(char *&p1, char *p2, char *&res)
{
	// 添加了转换到大写字母
	for (; p1 != (p2+1); *res++=*p1++) {}
}
...
void SQLFormatter::_moveOneWord(char *&p1, char *&end, char *&res)
{
	//cerr << "*** Moving 1 word...\n";
	//cerr << "from p1 : " << p1 << "\n";
	//cerr << "word : ";
...
void SQLFormatter::_handleMaxLookaheadReached(char *&p1, char *&p2, char *&res, int indent, bool &lastWasToken, int &lookahead)
{
...
void SQLFormatter::_handleTokenOrEndline(char *&p1, char *&p2, char *&res, int &indent, bool &lastWasToken, int &lookahead, bool isFirstWord)
{
...
void SQLFormatter::_handleSubQuery(char *&p1, char *&p2, char *&res, int indent, bool &lastWasToken, int &lookahead)
{
	//cerr << "--------------------------------\n";
	//cerr << "-          SUBQUERY !          -\n";
	//cerr << "--------------------------------\n";
...
void SQLFormatter::_handleIncreaseLookahead(char *&p1, char *&p2, char *&res, int indent, bool &lastWasToken, int &lookahead)
{
	p1;
	lastWasToken;
	indent;
...
void SQLFormatter::formatSql(char *sqlString, char *dest)
{
...
bool SQLFormatter::sameLength(char *str1, char *str2)
{
	int strlen1 = 0;
	int strlen2 = 0;
	for (;*str1; str1++)
...
void SQLFormatter::flattenSql(char *sqlString, char *dest)
{  
	char *q = dest;
	char *p = sqlString;
	_moveWhileSpaces(p);
