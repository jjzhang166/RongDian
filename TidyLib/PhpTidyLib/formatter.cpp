...
/* runs formatter */
void PhpFormatter::Run(string &strOut)
{
	/* run parser */
	parser->Parse();
...
/* gets indent string */
string PhpFormatter::GetIndent(int add, bool half)
{
	int number = (indent + add) * settings->indent_number + settings->first_indent_number;
...
void PhpFormatter::AddAssignment(CNode *variable, CNode *semicolon)
{
	...
	if (!assignments.empty())
	{
		...
		do
		{
			...
			if (node == variable)
			{
				...
				(node->type != NT_LONG_COMMENT) ) //really (?)
			{
...
void PhpFormatter::FormatAssignments(void)
{
	SAssignment top;
...
void PhpFormatter::FormatOpenCurlyBracket(CNode **node, bool function_class)
{
	bool half = false;
	bool add  = false;
...
void PhpFormatter::FormatCloseCurlyBracket(CNode *node)
{
	/* prepend newline */
...
CNode *PhpFormatter::MoveShortComment(CNode *node) //NOTE: changing!
{
...
void PhpFormatter::FormatLongComment(CNode *node)
{
	const char *p = node->token.c_str() + 2;
	const char *end = p + node->token.length() - 4;
...
void PhpFormatter::AddStructureComment(CNode *expr, CNode *node) //NOTE: changing!
{
	string comment  = "//";
	const char *p = expr->token.c_str();
	const char *end = p + expr->token.length();
	char str = 0;
